...
enum Toolbox {
    ...
    static func makeQRCode(for path: String) throws -> URL {
        ...
        guard let data = text.data(using: .utf8),
            ...
            throw SimpleError("无法生成二维码")
        }
        filter.setValue(data, forKey: "inputMessage")
        filter.setValue("M", forKey: "inputCorrectionLevel")
        guard var ci = filter.outputImage else { throw SimpleError("二维码生成失败") }
    ...
    static func zip(_ paths: [String]) throws -> URL {
        guard let first = paths.first else { throw SimpleError("没有选中文件") }
        ...
        guard r.code == 0 else { throw SimpleError("压缩失败：\(r.out)") }
    ...
    static func unzip(_ paths: [String]) throws {
        for p in paths where (p as NSString).pathExtension.lowercased() == "zip" {
            ...
            guard r.code == 0 else { throw SimpleError("解压失败：\(r.out)") }
        }
    }
    ...
    static func convertImage(_ paths: [String], to fmt: ImageFormat) throws {
        for p in paths {
            guard let src = CGImageSourceCreateWithURL(URL(fileURLWithPath: p) as CFURL, nil),
                ...
                throw SimpleError("无法读取图片：\(PathUtils.lastComponent(p))")
            }
            let parent = (p as NSString).deletingLastPathComponent
            ...
            guard let dest = CGImageDestinationCreateWithURL(outURL as CFURL, fmt.uti as CFString, 1, nil) else {
                throw SimpleError("无法创建目标：\(fmt.displayName)")
            }
            CGImageDestinationAddImage(dest, cg, nil)
            if !CGImageDestinationFinalize(dest) {
                throw SimpleError("转换失败：\(PathUtils.lastComponent(p))")
            }
        }
    }
    ...
    static func info(for path: String) -> String {
        ...
        var lines = ["名称：\(ns.lastPathComponent)", "路径：\(path)"]
        let attrs = (try? fm.attributesOfItem(atPath: path)) ?? [:]
        var isDir: ObjCBool = false
        fm.fileExists(atPath: path, isDirectory: &isDir)
        lines.append("类型：\(isDir.boolValue ? "文件夹" : "文件")")
        if let size = attrs[.size] as? Int64 { lines.append("大小：\(ByteFormat.human(size))") }
        let df = DateFormatter(); df.dateFormat = "yyyy-MM-dd HH:mm:ss"
        if let c = attrs[.creationDate] as? Date { lines.append("创建：\(df.string(from: c))") }
        if let m = attrs[.modificationDate] as? Date { lines.append("修改：\(df.string(from: m))") }
        if let perm = attrs[.posixPermissions] as? Int { lines.append(String(format: "权限：%o", perm)) }
    ...
    static func makeAlias(_ paths: [String]) throws {
        for p in paths {
            ...
            let existing = Set((try? fm.contentsOfDirectory(atPath: parent)) ?? [])
            let name = FileName.unique(base: "\(base) 替身", ext: "", existing: existing)
    ...
    static func makeSymlink(_ paths: [String]) throws {
        for p in paths {
            ...
            let base = (p as NSString).lastPathComponent
            let existing = Set((try? fm.contentsOfDirectory(atPath: parent)) ?? [])
            let name = FileName.unique(base: "\(base) 链接", ext: "", existing: existing)
